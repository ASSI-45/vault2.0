...
**_Note!:_** Any value apart from nil or false is interpreted as true.
...
`Div` and `rem` just functions, who need value's to be passed into.
...
## Variable scope
-- -
Elixir uses lexical scoping, and modules define a scope for local variables, but these
are accessible only at the top level of that module, and not in functions defined
in the module. 

In elixir a common block is the `do` block.
```
if (condition) do
	# something
end
...
**_Note!_**; However, Elixir thinks this is a risky way to write code. In particular, it’s easy
to forget to initialize line_no outside the block, but to then rely on it having a
value after the block. For that reason, **you’ll see a warning**.

### the with expression
It creates an local scope for the variables to be used in. And won't effect any other variables outside of its own scope. The `with` statement also allows for quick and easy pattern matching. 

```elixir
with {:ok, x} <- {:ok, 5} do
		IO.write(x*2)
	else
		IO.write("Failing match")
end
```

Think of it this way. It will execute the following if block if all the matches succeed, and go the the `else` statement inside the `do` block if one fails. And only if one of the matches fails.
-- -
The next part is [[5. Anonymous Functions]].
